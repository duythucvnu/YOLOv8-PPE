...
class DCNv3_pytorch(nn.Module):
    ...
    def forward(self, input):
        ...
        x1 = self.dw_conv(x1).permute(0, 2, 3, 1)  # N H W C
...
class DCNv3(nn.Module):
    def __init__(
        ...
        if channels % group != 0:
            raise ValueError(
                f'channels must be divisible by group, but got {channels} and {group}')
        _d_per_group = channels // group
        # dw_kernel_size = kernel_size if dw_kernel_size is not None
        dw_kernel_size = dw_kernel_size if dw_kernel_size is not None else kernel_size
        # you'd better set _d_per_group to a power of 2 which is more efficient in our CUDA implementation
        if not _is_power_of_2(_d_per_group):
            warnings.warn(
        ...
        self.channels = channels
        self.kernel_size = kernel_size
        self.dw_kernel_size = dw_kernel_size  # dw_kernel_size = kernel_size if dw_kernel_size is not None
        self.stride = stride
        self.dilation = dilation
        self.pad = pad
        self.group = group
        ...
        self.dw_conv = nn.Sequential( # conv nor act remove norm act in DCNV4
            nn.Conv2d(
                channels,
                channels,
                kernel_size=dw_kernel_size,
    ...
    def forward(self, input):
        ...
        x1 = input.permute(0, 3, 1, 2)  # N C H W
        x1 = self.dw_conv(x1).permute(0, 2, 3, 1)  # N H W C
        offset = self.offset(x1)  # N H W C
        mask = self.mask(x1)  # N H W C
